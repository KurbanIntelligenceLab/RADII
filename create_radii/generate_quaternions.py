"""
Quaternion rotation generation for RADII benchmark.

Uses create_radii.config settings:
...
_BUDGETS: Optional[Dict[str, int]] = None
...
def _process_train_id(xyz_path: Path) -> Tuple[Dict[str, int], np.ndarray]:
    ...
    return RadiiQuaternionGenerator._process_train_id_impl(xyz_path)


def _process_ood(xyz_path: Path) -> Dict[str, int]:
    """Worker target for pass 2 (OOD). Must stay module-level for pickling."""
    return RadiiQuaternionGenerator._process_ood_impl(xyz_path)


class RadiiQuaternionGenerator:
    """
    Quaternion rotation generator for the RADII benchmark.

    Uses create_radii.config (TARGET_TOTAL_FILES, SPLIT_FRACTIONS,
    MAX_ROTS_PER_FILE, ANGLE_BY_SPLIT, etc.) via radii_config.
    """
    ...
    def _split_for_r(r: int) -> str:
        if r in radii_config.r_splits["ID"]:
            return "ID"
        if r in radii_config.r_splits["OOD"]:
            return "OOD"
        return "train"
    ...
    def _compute_budgets_from_target(
        ...
        """Compute per-file rotation budgets to hit TARGET_TOTAL_FILES."""
        ...
        for p in xyz_files:
            ...
            if not m:
                continue
            r = int(m.group(2))
            if r not in radii_config.r_values:
                continue
            base_counts[RadiiQuaternionGenerator._split_for_r(r)] += 1
        ...
        desired = {
            k: int(
                round(radii_config.TARGET_TOTAL_FILES * radii_config.SPLIT_FRACTIONS[k])
            )
            for k in radii_config.SPLIT_FRACTIONS
        }
        delta = radii_config.TARGET_TOTAL_FILES - sum(desired.values())
        if delta != 0:
            desired["train"] += delta

        # Compute budgets (rotations per base file)
        budgets = {"train": 0, "ID": 0, "OOD": 0}
        for split in ("train", "ID", "OOD"):
            ...
            k = min(k, radii_config.MAX_ROTS_PER_FILE[split]["base"])
            budgets[split] = int(k)
    ...
    def _expected_counts_from_budgets(
        base_counts: Dict[str, int], budgets: Dict[str, int]
    ) -> Dict[str, int]:
        """Compute expected output counts from budgets."""
        return {
            split: base_counts[split] * (1 + budgets[split])
            for split in ("train", "ID", "OOD")
        }

    @staticmethod
    def _process_train_id_impl(xyz_path: Path) -> Tuple[Dict[str, int], np.ndarray]:
        """Process a single file for train or ID split."""
        global _TRAIN_GRID, _EXCLUDE_QS, _BUDGETS, _OUTPUT_DIR
        ...
        split = RadiiQuaternionGenerator._split_for_r(r)
        if split == "OOD":
            return counts, id_quats_out

        num_atoms, elements, coords = RadiiQuaternionGenerator._load_xyz(xyz_path)

        out_dir = _OUTPUT_DIR / material / f"R{r}" / "xyz"
        out_dir.mkdir(parents=True, exist_ok=True)

        # Save original (unrotated)
        RadiiQuaternionGenerator._save_xyz_file(
            coords,
            elements,
            num_atoms,
            r,
        ...
        spacing = radii_config.ANGLE_BY_SPLIT[split]["base"]
        ...
        if split == "train":
            ...
            if grid.shape[0] < k:
                raise RuntimeError(
                    ...
                    "Increase MAX_ROTS_PER_FILE or train grid size."
                )
            rng = np.random.default_rng(
                RadiiQuaternionGenerator._stable_seed(material, r, "train", spacing)
            )
            idx = rng.choice(grid.shape[0], size=k, replace=False)
            all_quats = grid[idx]
        else:  # ID
            seed = RadiiQuaternionGenerator._stable_seed(material, r, "ID", spacing)
            all_quats = RadiiQuaternionGenerator._sample_with_exclusion(
                spacing,
                seed,
                k,
                exclude_Q=_EXCLUDE_QS,
                margin_deg=radii_config.MARGINS["ID"],
                left_mul=radii_config.ID_OFFSET,
            )

        unique_qs = RadiiQuaternionGenerator._unique_rotations(
            coords, all_quats, tol=radii_config.coord_tolerance
        )
    ...
    def _process_ood_impl(xyz_path: Path) -> Dict[str, int]:
        """Process a single file for OOD split."""
        ...
        spacing = radii_config.ANGLE_BY_SPLIT["OOD"]["base"]
        seed = RadiiQuaternionGenerator._stable_seed(material, r, "OOD", spacing)
        ...
        all_quats = RadiiQuaternionGenerator._sample_with_exclusion(
            ...
            k,
            exclude_Q=_EXCLUDE_QS,
            margin_deg=radii_config.MARGINS["OOD"],
            left_mul=radii_config.OOD_OFFSET,
        )
    ...
    def generate(self) -> Dict[str, int]:
        ...
        print(
            f"  Train : {budgets['train']} @ "
            f"{radii_config.ANGLE_BY_SPLIT['train']['base']}°"
        )
        print(
            f"  ID    : {budgets['ID']} @ {radii_config.ANGLE_BY_SPLIT['ID']['base']}°"
        )
        print(
            f"  OOD   : {budgets['OOD']} @ "
            f"{radii_config.ANGLE_BY_SPLIT['OOD']['base']}°"
        )
        print()
        ...
        print(
            f"  Total : {total_expected} (target {radii_config.TARGET_TOTAL_FILES})\n"
        )

        # Build train grid
        train_spacing = radii_config.ANGLE_BY_SPLIT["train"]["base"]
        train_grid_size = radii_config.MAX_ROTS_PER_FILE["train"]["base"]
        train_grid = {
            train_spacing: self._generate_uniform_rotations(
                train_spacing,
                self._stable_seed(
                    radii_config.GLOBAL_SEED, "train-grid", train_spacing
                ),
                train_grid_size,
            )
        }
        ...
        print("=== PASS 1: generating Train + ID ===")
        actual = {"train": 0, "ID": 0, "OOD": 0}
        all_id_quats: List[np.ndarray] = []

        with ProcessPoolExecutor(
            max_workers=radii_config.max_workers,
        ...
        print(f"  Total : {total_actual} (target {radii_config.TARGET_TOTAL_FILES})")

        if (
            abs(total_actual - radii_config.TARGET_TOTAL_FILES)
            > 0.10 * radii_config.TARGET_TOTAL_FILES
        ):
            print("\n[NOTE] Total differs from target by >10%. Common reasons:")
            print("  - Dedup collisions (high symmetry can collapse rotations).")
            print("  - Uneven base_counts or low per-file budgets.")
            print(
                "Try adjusting TARGET_TOTAL_FILES, SPLIT_FRACTIONS, "
...
def run_radii_quaternions(
    xyz_files: List[Path], output_root: Path, label: str = "radii"
) -> Dict[str, int]:
    """
    Run RADII quaternion generation.

    Uses create_radii.config for TARGET_TOTAL_FILES, MAX_ROTS_PER_FILE, etc.
    Returns actual counts per split.
    """
    gen = RadiiQuaternionGenerator(xyz_files, output_root, label=label)
    return gen.generate()

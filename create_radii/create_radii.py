#!/usr/bin/env python3
"""
Create radii dataset from raw data.

Takes radii_raw (directory or zip file) as input and outputs radii/ with:
- unit_cells/  – CIF files
- quaternions/ – rotated XYZ structures (Material/Rx/xyz/rot_*.xyz)

Uses create_radii.config and create_radii.generate_quaternions (TARGET_TOTAL_FILES,
...
def extract_zip_if_needed(raw_data_path: Path) -> tuple[Path, bool]:
    ...
    if raw_data_path.suffix.lower() in (".zip", ".zipx"):
        ...
        temp_extract = tempfile.mkdtemp(prefix="radii_raw_")
...
def create_radii(raw_data_dir: str = "radii_raw", output_dir: str = "radii") -> bool:
    """
    Create radii dataset from raw data (directory or zip file).

    Args:
        raw_data_dir: Path to raw data directory or zip file (radii_raw or radii_raw.zip)
        output_dir: Path to output radii directory
    """
    print("\n" + "=" * 60)
    print("Creating radii dataset from raw data")
    print("=" * 60)
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("radii dataset created successfully!")
        print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description="Create radii dataset from raw data (radii_raw dir or zip)",
...
Examples:
    ...
    parser.add_argument(
        ...
        "-r",
        default="radii_raw",
        help="Path to raw data directory or zip file (default: radii_raw)",
    )
    parser.add_argument(
        "--output",
        "-o",
        default="radii",
        help="Path to output directory (default: radii)",
    )

    args = parser.parse_args()
    success = create_radii(
